...
cd /root/rhce/services 2>/dev/null || true
command -v ansible-playbook >/dev/null || echo "  ${C_Y}note${C_0} ansible-core not installed"
command -v systemctl >/dev/null || echo "  ${C_Y}note${C_0} systemd not available — service state checks will fail"
...
check "chronyd is enabled" bash -c '[ "$(systemctl is-enabled chronyd 2>/dev/null)" = enabled ]'
check "chronyd is active" bash -c '[ "$(systemctl is-active chronyd 2>/dev/null)" = active ]'
