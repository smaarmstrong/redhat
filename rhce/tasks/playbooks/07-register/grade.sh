...
cd /root/rhce/register 2>/dev/null || true
...
check "whoami.txt exists" test -f /root/rhce/register/whoami.txt
check_eval "whoami.txt contains the command output (root)" 'grep -qx "root" /root/rhce/register/whoami.txt'
grade_summary
